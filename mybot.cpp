...
public:
  // Constructor
  MyBot(std::string &token) : BasicBot(token, "MyBot", this) {}
...
void processMessage(MyBot &bot, const cppgram::types::Message &message) {
  // Answer with the text sent by the user
  bot.sendMessage("Hello folks!");
}

// Function called on /start messages
void startCommand(MyBot &bot, const cppgram::types::Message &message) {
  // Answer
  bot.sendMessage("Hello folks!");
}
